...
def download_cover(
    ...
    status("Подготовка обложки...")

    original_file = os.path.join(
        output_folder,
        base_name + ".cover_source"
    )
    ...
    try:
        ...
        if not os.path.exists(cover_file):
            ...
            print()
            print(
                "Обложка не была создана."
            )

            return None

        if os.path.getsize(cover_file) == 0:
...
def embed_cover(
    ...
    status("Встраивание обложки в MP3...")

    temp_mp3 = mp3_filepath + ".cover_tmp.mp3"
    ...
    try:
        ...
        if not os.path.exists(temp_mp3):
            ...
            print()
            print(
                "FFmpeg не создал новый MP3."
            )

            return False

        if os.path.getsize(temp_mp3) < MIN_FILE_SIZE:

            print()
            print(
                "Полученный MP3 некорректен."
            )
        ...
        # ----------------------------------------------------
        # Дополнительная проверка наличия обложки
        # ----------------------------------------------------
        ...
        check_command = [
            ...
            "v:0",
            "-show_entries",
            "stream=codec_name,width,height,disposition",
            "-of",
            "json",
            mp3_filepath
        ]
        ...
        if check.returncode != 0:
            ...
            print()
            print(
                "Не удалось проверить встроенную обложку."
            )
