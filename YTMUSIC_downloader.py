...
def download_cover(
    ...
    status("Получение обложки из YouTube Music...")

    safe_title = safe_filename(title)

    temp_base = os.path.join(
        output_folder,
        f"cover_temp_{safe_title}"
    )

    downloaded_cover = os.path.join(
        output_folder,
        f"cover_source_{safe_title}"
    )

    output_cover = os.path.join(
        output_folder,
        f"cover_{safe_title}.jpg"
    )
    ...
    try:
        ...
        for filename in os.listdir(output_folder):
            ...
            if filename.startswith(
                os.path.basename(temp_base)
            ) or filename.startswith(
                os.path.basename(downloaded_cover)
            ):
        ...
        if not possible_files:
            ...
            print()
            print("ОШИБКА: обложка не найдена.")
        ...
        source_cover = possible_files[0]

        # ----------------------------------------------------
        # FFmpeg: приводим к ровно 720x720
        # ----------------------------------------------------

        status("Подготовка обложки 720×720...")
        ...
        if (
            result.returncode != 0
            or
            not os.path.exists(output_cover)
        ):

            print()
            print("ОШИБКА: не удалось подготовить обложку.")
        ...
        try:
            ...
            if not streams:
                ...
                print()
                print(
                    "ОШИБКА: FFprobe не видит изображение."
                )
            ...
            if width != 720 or height != 720:
                ...
                print()
                print(
                    f"ОШИБКА: неправильный размер "
                    f"обложки: {width}×{height}"
                )
            ...
            print()
            print(
                "ОШИБКА: не удалось проверить размер "
                "обложки."
            )

            return None

        print("Обложка готова: 720×720.")
...
def verify_embedded_cover(mp3_filepath):
    ...
    status("Проверка встроенной обложки...")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print()
            print(
                "ОШИБКА: ffprobe не смог прочитать MP3."
            )
        ...
        print()
        print(
            "ОШИБКА: в MP3 не найден "
            "корректный attached picture."
        )
        ...
        print()
        print(
            "ОШИБКА: ffprobe вернул некорректные данные."
        )
...
# ============================================================
# ВСТРАИВАНИЕ ОБЛОЖКИ И МЕТАДАННЫХ
# ============================================================
...
def embed_cover(
    ...
    status("Встраивание обложки и метаданных...")

    if not os.path.exists(mp3_filepath):

        print()
        print("MP3-файл не найден.")
    ...
    try:
        ...
        # ВАЖНО:
        #
        # -map 0:a:0       = оригинальный звук
        # -map 1:v:0       = картинка
        # -c:a copy        = звук не перекодируем
        # -c:v mjpeg       = JPEG внутри MP3
        # attached_pic     = именно обложка
        # id3v2_version 3  = максимально совместимый ID3
        # ----------------------------------------------------
        ...
        if (
            result.returncode != 0
            or
            not os.path.exists(temp_mp3)
        ):

            print()
            print(
                "ОШИБКА: FFmpeg не смог встроить обложку."
            )
        ...
        # ----------------------------------------------------
        # Проверяем ВРЕМЕННЫЙ MP3 до замены оригинала
        # ----------------------------------------------------

        if not verify_embedded_cover(
            temp_mp3
        ):

            print()
            print(
                "ОШИБКА: обложка не была "
                "корректно встроена."
            )
        ...
        # ----------------------------------------------------
        # Финальная проверка уже готового MP3
        # ----------------------------------------------------
