...
def download_and_embed_cover(
    ...
    if not video_id:
        print("Не удалось определить ID трека.")
        return False

    temp_dir = tempfile.mkdtemp(
        prefix="ytmusic_cover_"
    )

    try:

        # ----------------------------------------------------
        # Получаем обложку через yt-dlp
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Подготавливаем изображение 720x720
        # ----------------------------------------------------

        square_cover = os.path.join(
            temp_dir,
            "cover_720.jpg"
        )
        ...
        if (
            result.returncode != 0
            or
            not os.path.exists(square_cover)
        ):
        ...
        status("Встраивание обложки в аудиофайл...")
...
def search_lrclib(
    ...
    if duration is None:
        ...
        print(
            "Недостаточно данных для поиска текста."
        )
