...
def evaluate_soundcloud_candidate(found_artist, found_title, candidate_duration, target_artist, target_title, target_duration):
    #Расширенный допуск длительности для SoundCloud (10 сек)
    ...
    #Ограниченный список только явных фейков/модификаций
    strict_modifiers = {
    ...
    #Проверка строгого совпадения (по полям или "Исполнитель - Трек" в title)
