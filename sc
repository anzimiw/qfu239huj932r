...
def clean_search_query(text):
    ...
    text = re.sub(r"\[feat\.[^\]]+\]", "", text, flags=re.I)
    text = re.sub(r"\(ft\.[^)]+\)", "", text, flags=re.I)
    text = re.sub(r"[()[\]{}]", " ", text)
    return re.sub(r"\s+", " ", text).strip()
...
def get_yandex_music_info(url):
    ...
    cover_url = None
    if cover_uri:
        cover_url = str(cover_uri).replace("%%", "720x720")
...
def candidate_text_score(filename, artist, title):
    ...
    if artist_ratio < 0.4 or title_ratio < 0.4:
        return -100000
    ...
    modifiers = {
        "nightcore", "remix", "slowed", "speed up", "sped up", "sped",
        "instrumental", "cover", "hardstyle", "phonk"
    }
...
def is_duration_acceptable(candidate_duration, target_duration, tolerance=DURATION_TOLERANCE):
...
def embed_cover(mp3_filepath, cover_url, artist, title, album=""):
    ...
    try:
        ...
        if "=w" in cover_url:
            url_to_fetch = re.sub(r'=w\d+-h\d+.*$', '=w720-h720-l90-rj', cover_url)
        ...
        img = img.resize((720, 720), resample_filter)
        ...
        print("Обложка (720x720) и теги добавлены.")
...
def search_lrclib(artist, title, album=None, duration=None):
    status("Поиск текста песни...")
    clean_art = clean_search_query(artist)
    clean_tit = clean_search_query(title)

    # 1. Попытка точного запроса через /api/get (без жесткой привязки к названию альбома)
    ...
    # 2. Гибкий поиск через /api/search (по ключевым словам)
    queries = [
    ...
    for query in queries:
        try:
            ...
            for item in results:
                ...
                item_track = normalize(item.get("trackName") or "")

                # Мягкая проверка: хотя бы частично совпадает название песни
                target_words = normalize_words(title)
                item_words = normalize_words(item_track)
                if target_words and not (target_words & item_words) and norm_target_title not in item_track and item_track not in norm_target_title:
                    continue

                # Если есть синхронизированный текст — сразу берем его
                if synced and synced.strip():
                    return synced.strip()

                # Если нет синхронизированного, запоминаем обычный как запасной
                if plain and plain.strip() and not plain_fallback:
...
def process_lrc(artist, title, album, duration, mp3_filepath):
    ...
    if is_synced:
        print("Найден синхронизированный текст с таймкодами.")
    else:
        print("Найден несинхронизированный обычный текст.")

    status("Сохранение текста...")
...
def evaluate_soundcloud_candidate(found_artist, found_title, candidate_duration, target_artist, target_title, target_duration):
    if target_duration is not None and candidate_duration is not None:
        if abs(candidate_duration - target_duration) > SOUNDCLOUD_DURATION_TOLERANCE:
    ...
    strict_modifiers = {
        ...
        "phonk", "instrumental", "cover", "nightcore", "hardstyle"
    }
    ...
    artist_match_ratio = len(target_artist_words & combined_words) / len(target_artist_words) if target_artist_words else 1.0

    artist_in_artist_field = artist_match_ratio >= 0.6
    title_in_title_field = (len(target_title_words & title_words) / len(target_title_words)) >= 0.6
    artist_and_title_in_title = (
        artist_match_ratio >= 0.6 and
        (len(target_title_words & title_words) / len(target_title_words)) >= 0.6
    )

    is_strict = (artist_in_artist_field and title_in_title_field) or artist_and_title_in_title

    score = (artist_match_ratio * 300) + (title_match_ratio * 500)
    if target_duration is not None and candidate_duration is not None:
        diff = abs(candidate_duration - target_duration)
        score += max(0, 300 - diff * 30)

    return True, score, is_strict


def fetch_soundcloud_results(query_str, client_id):
    try:
        response = requests.get(
            "https://api-v2.soundcloud.com/search/tracks",
            params={"q": query_str, "client_id": client_id, "limit": SOUNDCLOUD_SEARCH_RESULTS},
            headers=SOUNDCLOUD_HEADERS,
            timeout=SOUNDCLOUD_SEARCH_TIMEOUT
        )
        if response.status_code == 200:
            return response.json().get("collection") or []
    except Exception:
        pass
    return []
...
def search_soundcloud(artist, title, target_duration=None):
    ...
    for q in queries:
        collection = fetch_soundcloud_results(q, client_id)
        if not collection:
            continue
        ...
        if strict_candidates or fallback_candidates:
            break

    if strict_candidates:
        strict_candidates.sort(key=lambda x: x["final_score"], reverse=True)
        best = strict_candidates[0]
        print(f"Найден трек SoundCloud (точное совпадение): {best['artist']} - {best['title']}")
        return best

    if fallback_candidates:
        fallback_candidates.sort(key=lambda x: x["final_score"], reverse=True)
        best = fallback_candidates[0]
        print(f"Найден трек SoundCloud (гибкое совпадение): {best['artist']} - {best['title']}")
        return best
...
def download_from_soundcloud(soundcloud_url, filepath, target_duration=None):
    ...
    if not is_duration_acceptable(actual_duration, target_duration, SOUNDCLOUD_DURATION_TOLERANCE):
        try:
            os.remove(source_file)
        except Exception:
            pass
        print("Длительность скачанного файла значительно отличается от оригинальной.")
        return False
