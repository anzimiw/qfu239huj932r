...
# ============================================================
# Client ID получается один раз за запуск программы
# и затем используется повторно для всех последующих
# поисковых запросов SoundCloud.
...
def search_lrclib(
    ...
    # 1. Попытка точного запроса через /api/get
    ...
    # 2. Гибкий поиск через /api/search
    queries = [
    ...
    for query in queries:
        try:
            ...
            for item in results:
                ...
                if (
                    target_words
                    and not (target_words & item_words)
                    and norm_target_title not in item_track
                    and item_track not in norm_target_title
                ):
                    continue
...
def process_lrc(
    ...
    if is_synced:
        print(
            "Найден синхронизированный текст с таймкодами."
        )
    else:
        print(
            "Найден несинхронизированный обычный текст."
        )
...
def validate_audio_file(filename):
    if (
        ...
        or os.path.getsize(filename) < MIN_FILE_SIZE
    ):
        return False
...
# ============================================================
# ИЗМЕНЕНО:
# SoundCloud Client ID теперь кэшируется.
# ============================================================

def get_soundcloud_client_id():
    global SOUNDCLOUD_CLIENT_ID_CACHE

    # Если Client ID уже был получен ранее в этом запуске,
    # повторно обращаться к SoundCloud не нужно.
    ...
    try:
        ...
        for pattern in patterns:
            ...
            if match:
                SOUNDCLOUD_CLIENT_ID_CACHE = match.group(1)
                return SOUNDCLOUD_CLIENT_ID_CACHE
...
def search_soundcloud(
    ...
    print(
        "Подходящий трек на SoundCloud не найден."
    )
