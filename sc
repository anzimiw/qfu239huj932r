...
def evaluate_soundcloud_candidate(found_artist, found_title, candidate_duration, target_artist, target_title, target_duration):
    ...
    for mod in modifiers:
        if mod in norm_title and mod not in norm_requested:
            return False, 0

    # 3. Сопоставление слов названия
    ...
    title_overlap = len(target_title_words & title_words) / len(target_title_words)
    if title_overlap < 0.7:
        return False, 0

    score = title_overlap * 500

    # 4. Раздельная проверка исполнителя SoundCloud и оригинала
    artist_overlap = len(target_artist_words & artist_words) / len(target_artist_words)

    if artist_overlap >= 0.7:
        score += 1000
    else:
        artist_in_title_overlap = len(target_artist_words & title_words) / len(target_artist_words)
        if artist_in_title_overlap >= 0.7:
            score += 200
        else:
            return False, 0

    extra_words = title_words - (target_artist_words | target_title_words)
...
def search_soundcloud(artist, title, target_duration=None):
    ...
    for entry in collection:
        ...
        is_valid, score = evaluate_soundcloud_candidate(
