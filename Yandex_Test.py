...
def yandex_cover_url(
    ...
    url = re.sub(
        ...
        "/1000x1000",
        url
    ) if "avatars.yandex.net" in url and not re.search(
        r"/1000x1000$",
        url
    ) else url
...
def yandex_candidate_score(
    ...
    if (
        ...
        and found_duration is not None
    ):
...
def search_yandex_api(
    ...
    try:
        ...
        # ----------------------------------------------------
        # Иногда API может вернуть другую
        # структуру
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Если URL уже содержит точный ID,
        # отдаём ему приоритет
        # ----------------------------------------------------
...
def get_yandex_music_info(url):
    ...
    # --------------------------------------------------------
    # Получаем точный трек через поиск.
    #
    # Если ссылка содержит track_id,
    # он затем используется как абсолютный
    # приоритет среди найденных кандидатов.
    # --------------------------------------------------------

    result = search_yandex_api(
        "",
        "",
        None,
        track_id,
    ...
    # --------------------------------------------------------
    # Если поиск без текста не дал результата,
    # пробуем получить страницу через yt-dlp.
    # Это резерв только для метаданных.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Если поиск был вызван по ID и мы не имели
    # исходных artist/title, теперь всё равно
    # получаем окончательные данные.
    # --------------------------------------------------------
    ...
    print()
    print(
        "Метаданные Яндекс Музыки получены."
    )
...
def download_file(
    ...
    for attempt in range(
        ...
        try:
            ...
            # ------------------------------------------------
            # ВАЖНО:
            # НЕ отправляем принудительный Range.
            #
            # Некоторые MP3-серверы, включая MP3Party,
            # на такой запрос могут вернуть пустой ответ.
            # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Сначала создаём нормальную сессию.
            # ------------------------------------------------
...
def find_and_download_track(
    ...
    # 4. РЕЗЕРВ
    #
    # Для YouTube Music можно использовать yt-dlp.
    # Для Яндекс Музыки намеренно НЕ используем.
    # --------------------------------------------------------
