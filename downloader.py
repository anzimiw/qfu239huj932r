...
# Количество результатов SoundCloud,
# которые будут проверяться.
SOUNDCLOUD_SEARCH_RESULTS = 10

# Максимальное количество кандидатов SoundCloud,
# для которых дополнительно запрашиваются метаданные.
SOUNDCLOUD_METADATA_CHECK = 8
...
# SOUNDCLOUD
#
# Поиск выполняется через встроенный extractor yt-dlp.
# Используется scsearch.
#
# Сначала получаем несколько результатов.
# Затем подробно проверяем наиболее перспективные:
...
def search_soundcloud(
    ...
    try:
        ...
        if not entries:
            ...
            print(
                "SoundCloud: "
                "подходящие результаты не найдены."
            )

            return None

        # ----------------------------------------------------
        # Сначала оцениваем то, что уже есть
        ...
        preliminary = preliminary[
            :SOUNDCLOUD_METADATA_CHECK
        ]

        # ----------------------------------------------------
        # Подробно получаем метаданные кандидатов.
        # ----------------------------------------------------
        ...
        for candidate in preliminary:
            ...
            # Если flat search не дал длительность,
            # получаем её через отдельный запрос yt-dlp.
            ...
            # Пересчитываем текстовый score уже
            # по окончательным SoundCloud-метаданным.

            combined = (
                f'{candidate.get("uploader", "")} '
            ...
            if candidate_duration is not None:
                ...
                # Отсутствие длительности хуже,
                # чем наличие подтверждённой.
        ...
        # ----------------------------------------------------
        # Защита от явно неправильного результата.
        # ----------------------------------------------------
...
# ============================================================
# СКАЧИВАНИЕ SOUNDCLOUD ЧЕРЕЗ YT-DLP
# ============================================================
...
def download_soundcloud(
    ...
    try:
        ...
        # Если почему-то найдено несколько,
