...
# Максимальное время именно поиска SoundCloud.
# Если SoundCloud не отвечает за это время —
# сразу переходим к MP3Party.
SOUNDCLOUD_SEARCH_TIMEOUT = 12

# Максимальное время скачивания найденного SoundCloud-трека.
SOUNDCLOUD_DOWNLOAD_TIMEOUT = 90

# Сколько результатов SoundCloud анализировать.
SOUNDCLOUD_SEARCH_RESULTS = 8

# Максимальное количество кандидатов,
# для которых будем получать полную информацию.
SOUNDCLOUD_CANDIDATES = 4
...
# SOUNDCLOUD
#
# Поиск выполняется через установленный yt-dlp:
#
# scsearch8:исполнитель название
#
# Это значительно надёжнее, чем пытаться парсить
# HTML SoundCloud самостоятельно.
# ============================================================
...
def search_soundcloud(
    ...
    search_url = (
        f"scsearch{SOUNDCLOUD_SEARCH_RESULTS}:"
        f"{query}"
    )
    ...
    try:
        ...
        print(
            "SoundCloud не ответил "
            f"за {SOUNDCLOUD_SEARCH_TIMEOUT} секунд."
        )
    ...
    #
    # Это особенно важно, когда SoundCloud выдаёт несколько
    # одинаковых/похожих треков.
    # --------------------------------------------------------
    ...
    for candidate in candidates:
        ...
        # Если длительность уже есть в результате поиска,
        # дополнительный сетевой запрос не нужен.
        #
        # Если её нет — получаем точную информацию через
        # yt-dlp, но только для нескольких лучших кандидатов.
    ...
    if not best_candidate:
        ...
        print(
            "Подходящего SoundCloud "
            "кандидата не найдено."
        )
    ...
    if best_candidate["duration"] is not None:
...
def download_from_soundcloud(
    ...
    if not is_duration_acceptable(
        ...
        print(
            "Длительность SoundCloud-файла "
            "не совпадает с исходным треком."
        )
        ...
        print(
            "Получено:",
            format_duration(actual_duration)
        )
