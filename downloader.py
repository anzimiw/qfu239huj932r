...
def embed_cover(mp3_filepath, cover_url, artist, title, album=""):
    """Скачивает обложку, полностью очищает старые теги, конвертирует в JPEG и вшивает в ID3v2.3"""
    if not MUTAGEN_AVAILABLE:
        print("\n[ОШИБКА] Библиотека mutagen не установлена! Выполни: pip install mutagen")
        return
    ...
    if not PIL_AVAILABLE:
        print("\n[ОШИБКА] Библиотека Pillow не установлена! Выполни: pip install Pillow")
        return

    status("Вшивание обложки и тегов...")

    try:
        # 1. Загрузка обложки с фоллбэком (если high-res ссылка даст 404, скачает оригинал)
        ...
        # 2. Конвертация WebP/PNG в чистый JPEG
        ...
        # 3. Полное удаление старых/битых тегов у скачанного пиратского файла
        try:
        ...
        # 4. Запись новых чистых тегов в стандарте ID3v2.3
        tags = ID3()
        ...
        print("Обложка и теги успешно вшиты в файл!")
