...
# Подключение Pillow для конвертации картинки в JPEG
try:
...
def embed_cover(mp3_filepath, cover_url, artist, title, album=""):
    """Скачивает обложку, очищает старые теги, конвертирует в JPEG и вшивает в ID3v2.3"""
    if not MUTAGEN_AVAILABLE:
        print("\n[ОШИБКА] Библиотека mutagen не установлена! Установи: pip install mutagen")
        return
    ...
    if not PIL_AVAILABLE:
        ...
        print("Windows не видит обложку, так как YouTube отдаёт её в WebP.")
        print("Установи её командой в cmd: pip install Pillow")
        return

    status("Вшивание обложки и тегов...")

    try:
        # Запрашиваем высокое разрешение обложки YouTube
        cover_url = re.sub(r'=w\d+-h\d+.*$', '=w1200-h1200-l90-rj', cover_url)

        response = requests.get(cover_url, headers=HEADERS, timeout=15)
        if response.status_code != 200:
            print(f"Не удалось скачать обложку (код ответа: {response.status_code})")
            return

        # Конвертируем исходный WebP в чистый JPEG
        ...
        # Загружаем MP3 и сбрасываем старые обложки из пиратского файла
        try:
        ...
        # Сохраняем строго в формате ID3v2.3 (стандарт Windows и FL Studio)
        audio.save(v2_version=3)
        print("Обложка успешно сконвертирована в JPEG и вшита в файл!")
        ...
        try:
            ...
            with requests.Session() as session:
                ...
                if response.status_code not in (200, 206):
                    if attempt < retries:
                        status("Повторная попытка скачивания...")
                        time.sleep(1)
                        continue
                    return False
                ...
                print()
                print("Получено:", round(total / 1024 / 1024, 2), "МБ")

                if total < MIN_FILE_SIZE:
                    if attempt < retries:
                        status("Получен некорректный файл. Повторная попытка...")
                        time.sleep(1)
                        continue
                    return False
                ...
                if not validate_audio_file(temp_filename):
                    if attempt < retries:
                        status("Файл не прошёл проверку. Повторная попытка...")
                        time.sleep(1)
                        continue
                    return False
            ...
            if attempt < retries:
                status("Ошибка соединения. Повторная попытка...")
                time.sleep(1)
                continue
...
def search_mp3party(artist, title):
    ...
    try:
        ...
        for found_artist, song_id, found_title, _ in pattern.findall(text):
            if (
                normalize(found_artist) == wanted_artist
                and normalize(found_title) == wanted_title
            ):
                return {
                    "url": "https://dl2.mp3party.net/download/" + song_id,
...
def score_mp3tm_candidate(filename, artist, title):
    ...
    if artist_words:
        ratio = len(artist_matches) / len(artist_words)
        if ratio == 1:
            score += 300
        elif ratio >= 0.5:
            score += 120
    ...
    if title_words:
        ratio = len(title_matches) / len(title_words)
        if ratio == 1:
            score += 300
        elif ratio >= 0.5:
            score += 120
    ...
    if (wanted_artist + " - " + wanted_title) in name:
        score += 500

    if (wanted_title + " - " + wanted_artist) in name:
        score += 350

    if name == (wanted_artist + " " + wanted_title):
        score += 500
    ...
    modifiers = [
        "nightcore", "remix", "slowed", "sped", "speed", "bass",
        "type", "beat", "edit", "extended", "instrumental", "cover",
        "live", "acoustic", "rework", "version", "bootleg", "club",
        "hardstyle", "phonk", "prod"
    ]

    requested_text = wanted_artist + " " + wanted_title
...
def search_mp3tm(artist, title, target_duration=None):
    ...
    try:
        ...
        links = re.findall(r'https?://[^"\']+\.mp3(?:\?[^"\']*)?', text, re.I)
        links = list(dict.fromkeys(links))
        candidates = []

        for link in links:
            filename = clean_filename(link.split("/")[-1])
            score = score_mp3tm_candidate(filename, artist, title)
        ...
        for candidate in top_candidates[:10]:
            candidate["duration"] = get_duration(candidate["url"])

        if target_duration is not None:
            valid = [item for item in top_candidates if item["duration"] is not None]
            if valid:
                best = min(valid, key=lambda item: abs(item["duration"] - target_duration))
            else:
...
def search_audiostart(artist, title):
    ...
    try:
        ...
        links = re.findall(r'href=["\']([^"\']*?/getmp3/[^"\']+)["\']', text, re.I)
        links = list(dict.fromkeys(links))
        ...
        for link in links:
            try:
                ...
                decoded = normalize(unquote(decoded))
...
def find_and_download_track(artist, title, duration, output_folder):
    print()
    print("=" * 60)
    print("ТРЕК:", artist, "—", title)
    print("Длительность:", format_duration(duration))
    print("=" * 60)

    filename = f"{safe_filename(artist)} - {safe_filename(title)}.mp3"
    filepath = os.path.join(output_folder, filename)

    result = search_mp3party(artist, title)
    if result and download_file(
        result["url"], filepath, referer=result["referer"], retries=MP3PARTY_RETRIES
    ):
        return filepath

    result = search_mp3tm(artist, title, duration)
    if result and download_file(
        result["url"], filepath, referer=result["referer"], retries=2
    ):
        return filepath

    result = search_audiostart(artist, title)
    if result and download_file(
        result["url"], filepath, referer=result["referer"], retries=2
    ):
        return filepath
...
def process_single_track(url, output_folder):
    ...
    filepath = find_and_download_track(artist, title, duration, output_folder)
...
def process_playlist(url):
    ...
    output_folder = os.path.join(script_folder, playlist_title)
...
def main():
    ...
    print()
    url = input("Ссылка на трек или плейлист YouTube Music: ").strip()
    ...
    tracks_folder = os.path.join(script_folder, "tracks")
